...
const ItemContainer = () => {
  ...
  if (list.items && list.items.length > 0) {
    return (
      <>
        {list.items.map(item => (
          <Flex justifyContent="space-between" m="10px" key={item.id}>
            <Checkbox
              size="lg"
    ...
    return (
      <>
        <SkeletonText startColor="gray.700" noOfLines={1} skeletonHeight="5" />
        <Text mt="2px" mb="2px" background="gray.700">
          No items
        </Text>
        <SkeletonText
          startColor="gray.700"
